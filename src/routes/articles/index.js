...
async function profiles(server, options, done) {
  ...
  async function onDeleteArticle(req, reply) {
    const article = await articlesModel.deleteArticle(
      req.user.id,
      req.params.slug
    );
    if (!article) {
  ...
  async function onFavoriteArticle(req, reply) {
    const article = await articlesModel.favoriteArticle(
      req.user.id,
      req.params.slug
    );
    if (!article) {
  ...
  async function onUnfavoriteArticle(req, reply) {
    const article = await articlesModel.unfavoriteArticle(
      req.user.id,
      req.params.slug
    );
    if (!article) {
